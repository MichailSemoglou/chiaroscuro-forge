    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(image_path: str, *args, **kwargs):
            ...
            try:
                ...
                # If cache key generation fails for an expected file error, just compute without caching
            ...
            if len(cache._stats_cache) > cache.max_stats_cache:
                # Remove oldest entry
                oldest_key = min(cache._stats_cache.keys(), key=lambda k: cache._stats_cache[k][0])
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(preset_name: str, *args, **kwargs):
            ...
            if len(cache._preset_cache) > cache.max_preset_cache:
                oldest_key = min(
                    cache._preset_cache.keys(), key=lambda k: cache._preset_cache[k][0]
                )
