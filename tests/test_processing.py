...
class TestProcessImage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_image = test_rng.random((100, 100, 3))
        ...
        self.small_image = small_rng.random((5, 5, 3))
        ...
        gray_rng = np.random.default_rng(2)
        self.gray_image = gray_rng.random((100, 100))
    ...
    def test_processing_with_scaled_lab_color_preservation(self):
        """Scaled processing should preserve colors without shape mismatches."""
        processed = process_image(
            self.test_image_path,
            scale_factor=0.5,
        ...
        self.assertEqual(processed.shape[:2], (50, 50))
    ...
    def test_output_directory_creation(self):
        ...
        nested_output = os.path.join(self.temp_dir, "subdir1", "subdir2", "output.png")
...
class TestProcessImageEdgeCases(unittest.TestCase):
    ...
    def test_corrupted_image_file(self):
        ...
        self.assertTrue(
            "failed to load" in error_msg or "magic number" in error_msg or "cannot" in error_msg
        )
    ...
    def test_empty_file(self):
        ...
        img_path = os.path.join(self.temp_dir, "valid.png")
        io.imsave(img_path, img_as_ubyte(img))

        decoded_input = img_as_float(io.imread(img_path))
        ...
        self.assertLess(np.var(processed), np.var(decoded_input))
        # Should not be empty
        self.assertGreater(processed.size, 0)

    def test_high_denoise_sigma(self):
        """Test processing with high denoise sigma."""
        denoise_rng = np.random.default_rng(4)
        img = denoise_rng.random((50, 50, 3))
        img_path = os.path.join(self.temp_dir, "test.png")
        io.imsave(img_path, img_as_ubyte(img))

        decoded_input = io.imread(img_path)
